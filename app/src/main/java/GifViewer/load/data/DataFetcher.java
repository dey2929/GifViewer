...
import GifViewer.load.data.mediastore.*;
 ...
 *
 * <p> A new instance is
 * created per resource load by {@link GifViewer.load.model.ModelLoader}. {@link
 * #loadData(GifViewer.Priority, GifViewer.load.data.DataFetcher.DataCallback)}
 ...
 * {@link #loadData(GifViewer.Priority,
 * GifViewer.load.data.DataFetcher.DataCallback)}} is called, then so {@link #cleanup()}
 * will be called. </p>
 *
...
public interface DataFetcher<T> {
   ...
   */
  void loadData(RenderScript.Priority priority, DataCallback<? super T> callback);

  /**
   * Cleanup or recycle any resources used by this data fetcher. This method will be called in a
   * finally block after the data provided by {@link #loadData(GifViewer.Priority,
   * GifViewer.load.data.DataFetcher.DataCallback)} has been decoded by the
   * {@link GifViewer.load.ResourceDecoder}.
   *
   * <p> Note - this method will be run on a background thread so blocking I/O is safe. </p>
   */
  void cleanup();
  ...
  /**
   * Returns the {@link GifViewer.load.DataSource} this fetcher will return data from.
   */
