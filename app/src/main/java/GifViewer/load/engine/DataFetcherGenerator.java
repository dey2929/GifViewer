...
/**
 * Generates a series of {@link GifViewer.load.data.DataFetcher DataFetchers} using
 * registered {@link GifViewer.load.model.ModelLoader ModelLoaders} and a model.
 */
interface DataFetcherGenerator {
  /**
   * Called when the generator has finished loading data from a
   * {@link GifViewer.load.data.DataFetcher}.
   */
  ...
  /**
   * Attempts to a single new {@link GifViewer.load.data.DataFetcher} and returns true if
   * a {@link GifViewer.load.data.DataFetcher} was started, and false otherwise.
   */
  boolean startNext();
