...
import GifViewer.load.resource.drawable.DrawableTransitionOptions;
import GifViewer.load.resource.gif.GifDrawable;
import GifViewer.manager.ConnectivityMonitor;
import GifViewer.manager.ConnectivityMonitorFactory;
import GifViewer.manager.Lifecycle;
import GifViewer.manager.LifecycleListener;
import GifViewer.manager.RequestManagerTreeNode;
import GifViewer.manager.RequestTracker;
import GifViewer.manager.TargetTracker;
...
import GifViewer.request.target.ViewTarget;
import GifViewer.request.transition.Transition;
import GifViewer.util.Util;

import static GifViewer.request.RequestOptions.decodeTypeOf;
import static GifViewer.request.RequestOptions.diskCacheStrategyOf;
import static GifViewer.request.RequestOptions.skipMemoryCacheOf;
...
public class RequestManager implements LifecycleListener {
    ...
    RequestManager(
        ...
        final Context context = gifViewer.getGlideContext().getBaseContext();
        ...
        defaultRequestOptions = gifViewer.getGlideContext().getDefaultRequestOptions();
        requestOptions = defaultRequestOptions;

        gifViewer.registerRequestManager(this);
    }
    ...
    public void onTrimMemory(int level) {
        gifViewer.getGlideContext().onTrimMemory(level);
    }
    ...
    public void onLowMemory() {
        gifViewer.getGlideContext().onLowMemory();
    }
    ...
    public void onDestroy() {
        ...
        gifViewer.unregisterRequestManager(this);
    }
     ...
     *
     *
     * GifViewer.load.ResourceDecoder}s that can decode any subclass of {@link Drawable}.
     *
     ...
     *
     *
     * GifViewer.load.engine.DiskCacheStrategy#DATA}. As a result, specifying a
     * {@link GifViewer.load.engine.DiskCacheStrategy} on this request is generally not
     * recommended.
     *
     ...
     * {@link GifViewer.load.ResourceDecoder}s
    ...
    public <ResourceType> RequestBuilder<ResourceType> as(Class<ResourceType> resourceClass) {
        return new RequestBuilder<>(gifViewer.getGlideContext(), this, resourceClass);
    }
    ...
    private void untrackOrDelegate(Target<?> target) {
        ...
        if (!isOwnedByUs) {
            gifViewer.removeFromManagers(target);
        }
    }
